...
from app.services.morphology import generate_minus_words
...
def _assert_campaign_access(db: Session, user_id: int, campaign_id: int) -> Campaign:
    campaign = db.execute(
        select(Campaign).join(MPAccount, MPAccount.id == Campaign.account_id).where(
            Campaign.id == campaign_id, MPAccount.user_id == user_id
        )
    ).scalar_one_or_none()
    if campaign is None:
...
def _upsert_minus_words(db: Session, campaign_id: int, queries: list[str]) -> list[str]:
    if not queries:
        return []
    roots_map = generate_minus_words(queries)
    for root, source_queries in roots_map.items():
        ...
        if minus_word is None:
            minus_word = MinusWord(campaign_id=campaign_id, word_root=root, source_queries=source_queries)
            db.add(minus_word)
            continue
        existing = set(minus_word.source_queries if isinstance(minus_word.source_queries, list) else [])
        minus_word.source_queries = sorted(existing.union(source_queries))
    db.commit()
...
def list_queries(
    ...
    for row_query, row_label, campaign_name, row_marketplace in rows:
        hint = classify_query_default(ctr=row_query.ctr, impressions=row_query.impressions, orders=row_query.orders)
        result.append(
            SearchQueryOut(
                id=row_query.id,
        ...
        selected_queries = [
            ...
            for row in db.execute(
                select(QueryLabel.query).where(
                    QueryLabel.campaign_id == payload.campaign_id, QueryLabel.label == QueryLabelStatus.NOT_RELEVANT
                )
            ).all()
        ]
    ...
    for search_query, campaign_name, row_marketplace, row_label in rows:
        worksheet.append(
            [
                ...
                (row_label.value if row_label else classify_query_default(search_query.ctr, search_query.impressions, search_query.orders).value),
            ]
        )
